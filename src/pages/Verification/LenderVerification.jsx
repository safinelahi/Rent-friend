        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 sm:mb-16 gap-8">
           <div className="max-w-2xl">
              <div className="bg-accent/10 text-accent px-4 py-1.5 rounded-full border border-accent/10 inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-widest mb-4 sm:mb-6">
                 <FiZap size={12}/> Identity Protocol
              </div>
           ...
           {step <= totalSteps && (
             <div className="flex flex-col md:items-end gap-2 px-1">
                <p className="text-[9px] sm:text-[10px] font-black text-paragraph/40 uppercase tracking-widest">Protocol Step {step} of {totalSteps}</p>
        ...
        <div className="max-w-5xl mx-auto lg:mx-0">
            ...
            {step === 1 ? (
              ...
              >
                ...
                <div className="lg:col-span-4 bg-[#111] text-white p-8 sm:p-12 rounded-[32px] sm:rounded-[48px] flex flex-col justify-between relative overflow-hidden shadow-xl">
                   ...
                   <div className="relative z-10">
                    <h3 className="text-accent text-[9px] sm:text-[10px] font-black uppercase tracking-[0.4em] mb-6">Secure Vault</h3>
                    <p className="text-[11px] sm:text-xs font-medium leading-relaxed opacity-40 uppercase tracking-widest">Your identity data is encrypted via AES-256 and never shared with third parties.</p>
                   </div>
                </div>
              </motion.div>
            ) : step === 2 ? (
              ...
              >
                <div className="lg:col-span-7 bg-white p-8 sm:p-14 rounded-[32px] sm:rounded-[48px] border border-gray-100 shadow-sm text-center">
                   <div className="w-40 h-40 sm:w-48 sm:h-48 bg-[#F8F8F7] rounded-full mx-auto mb-8 sm:mb-10 border-4 sm:border-8 border-white shadow-inner flex items-center justify-center relative overflow-hidden group transition-all">
                      ...
                      <div className="absolute inset-0 bg-accent/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                         <button className="text-[8px] font-black uppercase tracking-widest bg-white px-4 py-2 rounded-full shadow-lg">Launch Camera</button>
                      </div>
                   </div>
                   <h2 className="text-2xl sm:text-3xl font-black tracking-tighter mb-4 uppercase">Face Scan</h2>
                   <p className="text-[11px] sm:text-sm font-medium text-paragraph/60 max-w-xs mx-auto leading-relaxed uppercase tracking-widest">Position your ID next to your face and face the lens directly.</p>
                </div>
              ...
              >
                ...
                <p className="text-sm sm:text-lg opacity-40 font-medium max-w-md mx-auto mb-10 sm:mb-12 uppercase tracking-widest leading-relaxed px-2">
                   Protocol check in progress. Audit typically completes within <span className="text-accent underline underline-offset-8">4 to 6 Hours.</span>
                </p>
                <button onClick={() => navigate('/lender/upload')} className="w-full sm:w-auto bg-accent text-txt px-12 py-5 sm:py-6 rounded-[20px] sm:rounded-[32px] font-black uppercase tracking-[0.3em] text-[10px] sm:text-xs shadow-2xl shadow-accent/20 hover:-translate-y-1 active:scale-95 transition-all">
                   Deploy Asset Catalog
                </button>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {step <= totalSteps && (
            <div className="mt-10 sm:mt-12 flex justify-between items-center px-2">
               ...
               <button 
                  onClick={() => setStep(s => s + 1)} 
                  className="bg-[#111] text-white px-8 sm:px-10 py-4 sm:py-5 rounded-[16px] sm:rounded-[24px] font-black uppercase tracking-[0.3em] text-[9px] sm:text-[10px] shadow-xl hover:bg-black flex items-center gap-3 transition-all active:scale-95"
                >
                  {step === totalSteps ? 'Finalize' : 'Next Protocol'} <FiArrowRight className="text-accent" />
               </button>
            </div>
          )}
        </div>
