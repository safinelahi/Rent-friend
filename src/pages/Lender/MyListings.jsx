...
const MyListings = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FDFDFC] flex flex-col items-center justify-center font-epilogue px-6 text-center">
      ...
      <p className="text-[10px] font-black uppercase tracking-widest text-paragraph/40">Loading Inventory...</p>
    </div>
  );
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            <div className="flex items-center gap-3 text-accent mb-6">
              <FiZap size={14} />
              <span className="text-[10px] font-black uppercase tracking-[0.5em]">My Inventory</span>
            </div>
            <h2 className="text-4xl md:text-7xl font-black tracking-tighter uppercase leading-none">
              My <br /> <span className="text-paragraph/20">Items.</span>
            </h2>
          </div>
          ...
          {listings.map((item) => (
            ...
            >
              ...
              <div className="aspect-square bg-[#F8F8F7] rounded-[32px] mb-8 overflow-hidden relative">
                ...
                <img src={getImageUrl(item.image)} alt={item.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-gray-50">
                   <button 
                    onClick={() => toggleStatus(item._id || item.id)}
                   ...
                   <button 
                     onClick={() => deleteListing(item._id || item.id)}
                     className="p-4 border border-gray-100 rounded-2xl text-paragraph/40 hover:text-red-500 hover:border-red-500 transition-all"
                   >
                      <FiTrash2 size={16} />
                   </button>
                </div>
