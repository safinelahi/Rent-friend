...
const ProductDetails = () => {
  ...
  if (!product) return (
    <div className="h-screen bg-[#F8F8F7] flex flex-col items-center justify-center font-epilogue">
  ...
  const handleBooking = () => {
    const bookingData = {
      tier: selectedTier,
      totalPrice: getDisplayPrice(),
      pickup: pickupDate || "Not Set",
      return: returnDate || "Not Set",
      days: selectedTier === "1" ? 1 : selectedTier === "2" ? 2 : 7
    };
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-8">
          <div className="max-w-4xl">
             <div className="flex items-center gap-3 mb-6">
                {/* PRODUCT IDENTIFIER - Distinct Asset ID Tag */}
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-8 space-y-12">
            ...
            <section className="space-y-8">
               ...
               <div className="w-full h-[450px] bg-white rounded-[32px] border border-gray-100 overflow-hidden relative shadow-sm">
                  <iframe
                    title="map"
            ...
            <div className="bg-white border border-gray-100 rounded-[32px] p-10 shadow-[0_40px_100px_rgba(0,0,0,0.03)]">
              <div className="flex items-baseline justify-between mb-8 pb-8 border-b border-gray-50">
                <div>
                  ...
                  <span className="text-paragraph text-[11px] font-black uppercase tracking-widest ml-2">/ Day</span>
                </div>
                <div className="text-[9px] font-black uppercase text-accent border border-accent/20 px-4 py-2 rounded-full flex items-center gap-2">
              ...
              <div className="grid grid-cols-3 gap-2 mb-10 bg-[#F1F1F0] p-1.5 rounded-[20px]">
                {["1", "2", "7"].map((tier) => (
                  <button 
                    key={tier} onClick={() => setSelectedTier(tier)}
                    className={`py-4 rounded-[16px] text-center transition-all ${selectedTier === tier ? 'bg-white text-txt shadow-md' : 'text-paragraph hover:text-txt'}`}
                  >
                    <p className="text-[10px] font-black uppercase">{tier} Day</p>
              ...
              <div className="space-y-4 px-2">
                 <div className="flex items-center gap-4 text-[10px] font-black text-paragraph/50 uppercase tracking-widest">
                    <FiCheckCircle size={16} className="text-accent" /> Verified Identity Lender
                 </div>
              </div>
            </div>

            {/* 2. PROFESSIONAL OWNER SECTION */}
               ...
               <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-[10px] font-black text-accent uppercase tracking-widest">
                       <FaStar size={10}/> {product.owner.rating} <span className="text-paragraph/40">({product.owner.reviews} reviews)</span>
                    </div>
                  </div>
               </div>
               <button className="w-full mt-8 py-4 rounded-[18px] border border-gray-100 text-[10px] font-black uppercase tracking-[0.2em] text-paragraph hover:bg-[#F1F1F0] transition-all">
                  8 Active Listings
               </button>
            </div>
